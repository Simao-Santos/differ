...
// When using got, the ECONNREFUSED error was constantly ocurring,
// probably because of some package interferences
...
async function getRequest(url) {
  ...
  try {
    // The content will be outputted as a buffer,
    // that will consequently be decoded to an UTF-8 string
    ...
    const resultingStrArr = stringContent.match(regex);

    if (resultingStrArr != null && resultingStrArr.length > 0) {
      // Gets the substring with the encoding
      const resultingStr = resultingStrArr[0];
      const encoding = resultingStr.substring(resultingStr.lastIndexOf('=') + 1, resultingStr.lastIndexOf('"'));
